...
node[:deploy].each do |application, deploy|
  ...
  if node[:bare_rails] && node[:bare_rails][layer_short_name] && node[:bare_rails][layer_short_name][:undeploy_command] && node[:bare_rails][layer_short_name][:undeploy_command] != ''
